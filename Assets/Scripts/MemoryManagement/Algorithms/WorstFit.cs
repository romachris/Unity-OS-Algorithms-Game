...
public class WorstFit : MemoryAlgorithm
{
    ...
    public bool checkAnswer()
    {
        if (selectedPartition.size >= selectedProcess.size)
        {
            ...
            float selectedPartitionRemainingSpace = selectedPartition.size - selectedProcess.size;

            for (int i = memoryPartitions.Length - 1; i >= 0; i--)
            {
                if (memoryPartitions[i].isAvailable && memoryPartitions[i].size >= selectedProcess.size && memoryPartitions[i] != selectedPartition)
                {
                    float partitionRemainingSpace = memoryPartitions[i].size - selectedProcess.size;
                    if (partitionRemainingSpace > selectedPartitionRemainingSpace)
                    {
                        Debug.Log("Better partition available");
                        //wrong choice
                        //give feedback that there is a more efficient partition available
    ...
    private void initFeedbackArray()
    {
        ...
        memoryWrong[3] = "Find the partition that waste the most space";
        memoryWrong[4] = "Check again the available partitions";
        memoryWrong[5] = "You have to choose the biggest available partition";
